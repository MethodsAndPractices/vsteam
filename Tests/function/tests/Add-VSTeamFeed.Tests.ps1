...
Describe 'VSTeamFeed' {
   ...
   Context 'Add-VSTeamFeed for organization' {
   ...
   Context 'Add-VSTeamFeed for organization' {
      ...
      it 'should add feed with description' {
         ## Act
         Add-VSTeamFeed -Name 'module' -ProjectName 'testProject'`
            -Description 'Test Module'

         ## Assert
         Should -Invoke Invoke-RestMethod -Exactly -Times 1 -Scope It -ParameterFilter {
            $Uri -eq "https://feeds.dev.azure.com/test/testProject/_apis/packaging/feeds?api-version=$(_getApiVersion Packaging)" -and
            $Method -eq 'Post' -and
            $Body -like '*"name":*"module"*'
         }
      }
