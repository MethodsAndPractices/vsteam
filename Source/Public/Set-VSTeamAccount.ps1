function Set-VSTeamAccount {
    ...
    process {
        ...
        if ($Profile) {
            ...
            if ((!$_pat) -and ([System.Environment]::OSVersion.Platform -like "win*") -and ($UsingWindowsAuth)) {
                Write-Verbose "Using Default Windows Credentials for authentication; no Personal Access Token required"
                $encodedPat = ''
                $token = ''
            }
        }
        if(([System.Environment]::OSVersion.Platform -like "win*") -and ($UsingWindowsAuth) -and $( $Account -like "*.visualstudio.com*" -or  $Account -like "https://dev.azure.com/*")) {
