class VSTeamWorkItemTypeCache {
   ...
   static [Void] Update() {
      ...
      if ($list -and [VSTeamWorkItemTypeCache]::WorkItemTypes.count -eq 0) {
         [VSTeamWorkItemTypeCache]::update($list, $projectName)
      }
   }
   ...
   static [object[]] GetCurrent([string]$ProcessName) {
